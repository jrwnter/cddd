...
def parallel_eval_qsar(model, step, hparams):
    ...
    process = mp.Process(target=eval_qsar, args=(step, embedding_array, dataset_array, label_array, fold_array, task_array, hparams))
    process.start()
    return process
...
def qsar_regression(emb, groups, labels):
    ...
    #SVR(kernel='rbf', C=10.0, epsilon=0.001)
    for train_index, test_index in logo.split(emb, groups=groups):
        clf.fit(emb[train_index], labels[train_index])
        y_pred = clf.predict(emb[test_index])
        y_true = labels[test_index]
...
# get a task column in infer csv...
def eval_qsar(step, embedding_array, dataset_array, label_array, fold_array, task_array, hparams):
    ...
    for dataset in datasets:
        ...
        if np.all(task_array[idxs] == "classification"):
            header += [dataset + "_" + measure for measure in ["accuracy", "f1", "roc_auc", "pr_auc"]]
            ...
            raise ValueError("Not a conistent task specification (classification or regression) for datset ", dataset)
